...
public class LoginController {
    ...
    public String regloginPage(Map map, @RequestParam(value = "next", required = false) String next){//不管是地址栏里的参数，还是表单里面的参数都可以用@RequestParam取得
    ...
    public String login(Map map,
        ...
        try{
            ...
            if(regMap.containsKey("ticket")) {
                ...
                cookie.setPath("/");//将ticket下发到客户端
                response.addCookie(cookie);
                if(!StringUtils.isEmpty(next)){//踩坑记录：因为没加！调试几个小时
                    return "redirect:"+next;
                }
                return "redirect:/";//跳到首页
            }else{
